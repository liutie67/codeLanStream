...
LanStream 是一个局域网媒体信息流系统（支持本地/NAS的私有化 TikTok/小红书）。当前处于阶段1：后端 MVP。完整的4阶段路线图记录在 `docs/` 中。
...
- `app/main.py` → FastAPI 应用，lifespan 中自动建表
...
阶段1（当前）：后端 MVP — 扫描、API、流传输
阶段2：Vue 3 + TailwindCSS 前端，瀑布流/滑动信息流
